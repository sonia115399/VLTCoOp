...
class DatasetWrapper(TorchDataset):
    def __init__(self, data_source, input_size, transform=None, is_train=False,
        ...
        to_tensor = []
        # Ensure Resize receives a (H, W) tuple so output has fixed spatial dims
        if isinstance(input_size, int):
            resize_size = (input_size, input_size)
        else:
            resize_size = input_size
        to_tensor += [T.Resize(resize_size, interpolation=interp_mode)]
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            ...
            # No transform provided: convert the PIL image to tensor using the
            # default to_tensor pipeline so downstream code always receives
            # an 'img' key.
        ...
        # Return the entire output dict so DataLoader collates into a dict
        # with keys like 'img', 'label', 'impath', etc., which trainers
        # expect (they access batch['img'], batch['label'], ...).
        return output
