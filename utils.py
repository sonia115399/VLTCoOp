    ...
    if cfg['load_pre_feat'] == False:
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(tqdm(loader)):
                    # Support different dataloader batch formats:
                    # - tuple/list: (images, target)
                    # - dict: {'img': images, 'label': target, ...}
...
def _check_and_coerce_cfg_value_type(replacement, original, key, full_key):
    ...
    try:
        # Python 2 compatibility: append (str, unicode) only if 'unicode' exists
        unicode_type = getattr(__builtins__, 'unicode', None)
        if unicode_type is not None:
            casts.append((str, unicode_type))  # type: ignore
    except Exception:
        pass
