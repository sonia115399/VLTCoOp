"""MaPLe trainer for the OpenAI CLIP backbone.
...
prompt learner while the frozen CLIP backbone supplies image and text features.
"""
...
class MaPLeCLIP(nn.Module):
    def __init__(self, backbone, classnames, options, method):
        ...
        self.prompt_learner = MaPLePromptLearner(backbone, classnames, options, method)
...
class MaPLeTrainer(TrainerX):
    ...
    def build_model(self):
        ...
        self.model = MaPLeCLIP(backbone, self.dm.dataset.classnames, self.options, self.method).to(self.device)
        if self.method == "PromptSRC":
...
@TRAINER_REGISTRY.register("MaPLe_CLIP")
class MaPLe(MaPLeTrainer):
    method = "MaPLe"
