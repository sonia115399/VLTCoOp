...
class KgCoOp_CLIP(TrainerX):
    ...
    def load_model(self, directory, epoch=None):
        ...
        # Build candidate filenames (priority order)
        candidates = []
        ...
        for name in names:
            ...
            for fname in candidates:
                p = osp.join(directory, name, fname)
                if osp.exists(p):
                    model_path = p
                    break
    ...
    def test(self, split=None):
        ...
        if split is None:
            # Fallback if TEST or SPLIT is missing from cfg
            try:
        ...
        # Compute macro F1 if sklearn is available; else 0.0
        macro_f1 = 0.0
        try:
            import numpy as _np
            from sklearn.metrics import f1_score as _f1
            labels_unique = _np.unique(_np.array(y_true)) if len(y_true) > 0 else []
            if len(labels_unique) > 0:
                macro_f1 = 100.0 * _f1(y_true, y_pred, average='macro', labels=labels_unique)
        except Exception:
            macro_f1 = 0.0
