...
class ClipAdapter_CLIP(FSCLIPmethod):
    ...
    def forward(self,
        ...
        for train_idx in range(self.cfg['train_epoch']):
            ...
            if acc > best_acc:
                ...
                # Save to cache dir
                torch.save(clip_ad_model.adapter.state_dict(), self.cfg['cache_dir'] + "/best_clipA_" + str(self.cfg['shots']) + "shots.pt")
                # Also save to output_dir if available
                if 'output_dir' in self.cfg:
                    if not os.path.exists(self.cfg['output_dir']):
                        os.makedirs(self.cfg['output_dir'])
                    
                    # Save in format compatible with Dassl/BiomedCoOp load_model
                    checkpoint = {
                        "state_dict": clip_ad_model.adapter.state_dict(),
                        "epoch": train_idx + 1,
                    }
                    torch.save(checkpoint, os.path.join(self.cfg['output_dir'], "model-best.pth.tar"))
        # Evaluation
    ...
    def load_model(self, directory, epoch=None):
        if not directory:
            print("Note that load_model() is skipped as no directory is given")
            return

        # Adapt path to where we saved best_model.pt
        model_path = os.path.join(directory, "best_model.pt")

        if not os.path.exists(model_path):
             # Try fallback to BiomedCoOp style if user points to deep structure
             # But here we saved it directly in output_dir
             raise FileNotFoundError('Model not found at "{}"'.format(model_path))

        print(f"Loading weights from {model_path}")
        # We need to load it into the adapter.
        # But this class doesn't hold the model permanently in self.
        # Wait, this class is a Trainer-like thing but inherits from FSCLIPmethod(nn.Module)
        # It's NOT a TrainerX from dassl.
        # generate_heatmap calls build_trainer -> TrainerX.load_model
        
        # So we need to make ClipAdapter_CLIP behave like a TrainerX if we want generate_heatmap to work?
        # generate_heatmap.py expects:
        # trainer = build_trainer(cfg)
        # trainer.load_model(...)
        # model = trainer.model
        
        # Currently ClipAdapter_CLIP is in trainers/ClipAdapter/clip_adapter_clip.py
        # It inherits FSCLIPmethod which inherits nn.Module.
        # It is NOT compatible with build_trainer(cfg) which expects a TrainerX subclass registered in TRAINER_REGISTRY.
        pass
...
def unpack_batch(batch):
    if isinstance(batch, dict):
        images = None
        for k in ('img', 'image', 'images', 'img0', 'img1'):
            if k in batch:
                images = batch[k]
                break
        target = None
        for k in ('label', 'labels', 'target', 'targets'):
            if k in batch:
                target = batch[k]
                break
        if images is None:
            raise ValueError(f"Could not find images in batch keys: {batch.keys()}")
        if target is None:
            raise ValueError(f"Could not find target in batch keys: {batch.keys()}")
