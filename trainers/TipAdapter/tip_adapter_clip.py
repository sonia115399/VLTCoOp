    ...
    def forward(self,
        ...
        for train_idx in range(self.cfg['train_epoch']):
            ...
            if acc > best_acc:
                ...
                # Also save to output_dir if available as model for generate_heatmap
                if 'output_dir' in self.cfg:
                    if not os.path.exists(self.cfg['output_dir']):
                        os.makedirs(self.cfg['output_dir'])
                    
                    # Save in format compatible with generate_heatmap custom loader
                    # We save adapter state dict AND cache_values because Tip-Adapter needs cache_values for inference
                    checkpoint = {
